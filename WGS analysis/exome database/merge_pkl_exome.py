...
for i in range(1,len(files)):
    ...
    if np.mod(i,10) == 0: #pass 10 pickles
        df.to_pickle(f'merge{file_counter:03}_exome.pkl') #save file           
        print(f'saved progress in merge {file_counter:03}_exome')
        df = pd.DataFrame(columns=columns) #update new dataframe
        file_counter +=1

df.to_pickle(f'merge{file_counter:03}_exome.pkl') #save file           
print(f'saved progress in merge {file_counter:03}_exome') # :03 three digits
